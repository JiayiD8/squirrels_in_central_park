...
```
As we mainly interested in how squirrels' behaviors varied, we would like to explore various other variables. About three third of the data are made on the ground level as people's vision are limited to catch squirrels in the tree. Clearly, ground plane squirrels show more running and foraging behaviors, while above ground squirrels exhibit more climbing and chasing activities. The pattern is reasonable that the high frequency of ground foraging aligns with squirrels' feeding patterns - they mainly eat nuts and pine cones which typically fall to the ground when mature. Eating behavior seems to be equally frequent to be sighted for both setting. 

### How do Eastern Gray squirrels interact with humans when being sighted?

Previous plot results that running and climbing might be solely related to the sighted location, hence, we want to take a closer look at the squirrels' foraging and eating behaviors and any possible correlation with their attitude when interacting with human. 
...
ggplot() +
  ...
  facet_wrap(~Behavior) +
  labs(
    title = "Distribution of Human Interactions by Behavior and Location",
    size = "Interaction Count",
    color = "Human Interation"
  ) +
