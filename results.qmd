...
```{r}
weather_data <- read.csv("/Users/sazle/Downloads/central_park_hourly.csv")
head(weather_data)
...
```
```{r}
interaction_distribution <- squirrel_data |>
  select(X, Y, Eating, Foraging, Running, Chasing, Climbing, Approaches, Indifferent, Runs.from) |>
  pivot_longer(
    cols = c(Eating, Foraging, Running, Chasing, Climbing),
    names_to = "Behavior",
    values_to = "Observed_Behavior"
  ) |>
  pivot_longer(
...
ggplot() +
  geom_sf(data = interaction_distribution, aes(color = Behavior), size = 3, shape = 1, alpha = 1) +
  facet_wrap(~Behavior)
  labs(
    title = "Frequency of Human Interactions by Behavior and Location",
    size = "Interaction Count",
...
# Convert squirrel_data to an sf object
squirrel_sf <- squirrel_data %>%
  st_as_sf(coords = c("X", "Y"), crs = 4326)  # Assuming WGS84 coords

# Determine bounding box and create hexagonal grid
bbox <- st_bbox(squirrel_sf)
# Adjust the 'cellsize' parameter as needed
# 'square = FALSE' creates hexagons
hex_grid <- st_make_grid(
  squirrel_sf,
  cellsize = c(0.001, 0.001), # Adjust to achieve desired hex size
  what = "polygons",
  square = FALSE
) %>%
  st_sf()

# Spatial join: find how many squirrels are in each hex
counts <- st_intersects(hex_grid, squirrel_sf)
hex_grid$Count <- lengths(counts)

# Plot the hex grid colored by frequency of squirrel sightings
ggplot(hex_grid) +
  ...
  labs(
    title = "Hex-Binned Frequency of Squirrel Sightings",
    x = "X Coordinate",
    y = "Y Coordinate"
  ) +
  theme_minimal()
